...
const ForgotPasswordAgency = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      const res = await axios.post(`${baseURL}/agency/forgot-password`, {
        identifier,
        newPassword,
        confirmPassword,
      });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${baseURL}/agency/forgot-password/verify-otp`,
        {
          identifier,
          otp,
        },
    ...
    <div className="flex items-center justify-center min-h-screen bg-[#111827] text-white px-2 sm:px-0">
      <div className="shadow-lg border border-gray-500 rounded-2xl px-4 py-6 sm:px-6 sm:py-8 w-full max-w-md bg-[#181F2A]">
        <div className="flex flex-col items-center gap-y-3 justify-center mb-4">
          ...
          <h1 className="font-bold text-xl sm:text-2xl text-center text-[#6917E0]">
        ...
        {!otpSent ? (
          <form onSubmit={handleSendOtp} className="space-y-4 mt-2">
            ...
            <button
              type="submit"
              className="w-full mt-2 bg-[#6917E0] hover:bg-[#8f4ff0] transition-colors duration-200 text-white py-2 rounded-lg font-semibold flex items-center justify-center"
              disabled={loading}
            >
          ...
          <form onSubmit={handleVerifyOtp} className="space-y-4 mt-2">
            ...
            <button
              ...
              disabled={otpLoading}
            >
              {otpLoading ? <ButtonLoading /> : "Reset Password"}
            </button>
          </form>
        )}
        <p className="text-sm text-center mt-4 text-gray-300">
          ...
          <Link
            to="/"
            className="text-[#6917E0] hover:underline font-medium transition-colors"
          >
            Login
          </Link>
        </p>
